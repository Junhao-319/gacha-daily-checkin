...
- 今日页默认使用游戏主视觉轮播，并带缓慢缩放和渐变切换动画。
- 点击首页游戏卡片进入独立任务页，页面背景会切换为该游戏对应的官方主视觉。
- 可连接本机 Steam Wallpaper Engine：自动读取 431960 工坊中的图片、GIF、MP4 和 WebM 壁纸。
- 今日页和每款游戏都能分别更换背景，也支持上传本地图片/视频或填写网络地址。
- 每款游戏拥有独立任务清单，任务可以新增、改名和删除。
- 游戏内部的每项任务可以单独勾选；全部任务完成后，该游戏当天记为完整打卡。
- 最近 126 天热力图按“完整完成的游戏数量”展示，点击日期可查看具体任务进度。
- 支持跟随系统、浅色和深色主题。
- 数据保存在浏览器 localStorage；桌面版还会同步到 %LOCALAPPDATA%\\GachaDailyCheckin\\state.json，重启后自动恢复上次背景与任务状态。
- 首次启动自带初音未来和已选游戏的主视觉动态轮播。
- 可安装到桌面或手机主屏，并在离线时打开。
...
启动器会把应用界面、全部背景图和 WebView2 运行组件打包进单个 `.exe`。双击后打开独立桌面窗口，不会跳出到外部浏览器；关闭窗口即退出应用。

桌面版会自动识别本机安装的游戏，但检测范围以 Windows 上存在的安装记录、目录和可执行文件为准，无法检测手机端的游戏。普通网页版本出于浏览器安全限制不能扫描本机，只能手动选择游戏。桌面版启动时会检查 GitHub Pages 上的背景图清单；版本更时会优先使用线上新图，网络不可用时自动回退到 EXE 内置图片。
